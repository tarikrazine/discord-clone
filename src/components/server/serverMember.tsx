...
function ServerMember(props: ServerMemberProps) {
    ...
    return (
        <button className={cn(
            ...
            params?.memberId === props.member.id ? "bg-zinc-700/20 dark:bg-zinc-700" : null
        )}>
            <UserAvatar src={props.member?.profile?.imageUrl!} className="h-8 w-8" />
            <p className={cn(
                "font-semibold text-sm text-zinc-500 group-hover:text-zinc-600 dark:text-zinc-400 dark:group-hover:text-zinc-300 transition",
                params?.memberId === props.member?.id ? "text-primary dark:text-zinc-200 dark:group-hover:text-white" : null
            )}>
